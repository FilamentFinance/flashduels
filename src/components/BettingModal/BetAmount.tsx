...
const BetAmount: React.FC<BetAmountProps> = ({ availableAmount, betAmount, setBetAmount }) => {
  ...
  return (
    <section className="flex flex-col justify-center mt-4 w-full">
      <div className="flex justify-between w-full text-base tracking-normal leading-none">
        <label
          htmlFor="betAmount"
        ...
        <div className="flex items-center h-full text-gray-400 w-[500px]">
          <span className="self-stretch my-auto w-[68px]">Available:{" "}</span>
          <span className="self-stretch my-auto">{availableAmount}</span>
          <button
            onClick={handleMaxClick}
            className="flex gap-2 items-start self-stretch my-auto text-xs font-medium tracking-normal text-center text-orange-400 whitespace-nowrap"
          >
            <span className="overflow-hidden gap-2 self-stretch px-2 py-1 rounded">
              Max
            </span>
          </button>
        </div>
          ...
          <div className="flex flex-1 shrink gap-2.5 items-start self-stretch px-4 py-2 my-auto w-full basis-0 min-w-[240px]">
            <div className="flex flex-col flex-1 shrink w-full basis-0 min-w-[240px]">
              ...
              <div className="mt-1 text-xs tracking-normal leading-loose text-gray-500">
                {parseInt(betAmount).toLocaleString()} Shares
              </div>
            </div>
          </div>
        </div>
